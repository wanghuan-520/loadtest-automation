...
// 自定义目标QPS: k6 run -e TARGET_QPS=50 payment-apple-subscription-qps-test.js
// 示例: k6 run -e TARGET_QPS=40 payment-apple-subscription-qps-test.js

// 自定义指标
const appleSubscriptionSuccessRate = new Rate('apple_subscription_success_rate');
const appleSubscriptionDuration = new Trend('apple_subscription_duration');
...
export const options = {
  ...
  //   'apple_subscription_success_rate': ['rate>0.99'],
  //   'apple_subscription_duration': ['p(95)<2000'],
  // },
};
...
export default function (data) {
  ...
  // 检查Apple订阅检查是否成功 - HTTP状态码200 + 业务code为20000
  ...
  // 记录Apple订阅检查指标 - 只有HTTP200且业务code为20000才算成功
  appleSubscriptionSuccessRate.add(isAppleSubscriptionSuccess);

  // 记录响应时间
  if (appleSubscriptionResponse.status === 200) {
...
export function setup() {
  return setupTest(
    config, 
    tokenConfig, 
    'payment/apple-subscription', 
    TARGET_QPS, 
    '/godgpt/payment/has-apple-subscription'
  );
}

// 测试清理阶段
export function teardown(data) {
  teardownTest('payment/apple-subscription', 'Apple订阅检查成功率、响应时间、QPS稳定性');
} 